# 20-00 — 🌃 AFTER HOURS

Playlist | 20:00 | 🌃 AFTER HOURS | HARD HIP-HOP · DARK CITY · NIGHT GRIND | 밤 여덟시 · 퇴근 · 도시 | Wavvy

---

## Series Status

- **착수**: 2026-04-25 (Phase 1 리서치 + Phase 2 /team 완료)
- **시리즈 유형**: **Wavvy 첫 힙합 시리즈** (기존 R&B·Lo-fi·Afrobeats·Neo-Soul·City Pop과 결 다름)
- **최종 트랙 수 목표**: **20곡**
- **배분**: **4:5:7:4** (Rage / K-Drill / 모던 하드코어 붐뱁 / 하드코어 트랩 젊은 씬)
- **워크플로우**: 15-00 모델 계승 — Suno PASS 곡 누적 → 충분히 쌓이면 DNA / 4막 구조 역추출
- **참고**: `report/2026-04-24_hard-hiphop-4axis-musical-deep.md` (신뢰도 88%, 소스 180+) / `meetings/2026-04-25_20-00-hard-hiphop-positioning.md`

---

## 🔥 핵심 원칙 (힙합 예외 룰)

### 가사 룰 (기존 Wavvy 룰 예외 적용)

`wavvy.md` §5 기본 가사 룰:
- ~~설명적 문장 금지~~
- ~~감정 직접 표출 금지~~
- ~~메타 표현 금지~~

→ **본 시리즈(힙합)는 장르 본질상 직접 서사·bar·메타가 DNA이므로 위 금지 항목 적용 제외.**
→ **유지**: 한국어 가사 / Chest-dominant 보컬 / Articulation (Brand DNA Constants)
→ **코어**: "시간의 감성을 음악으로 표현" — 저녁 8시 / 퇴근 후 / 밤의 도시 무드만 지키면 OK

### 장르 특이 룰
- **Rage**: Pitch-up baby voice 한국어 적용 시 받침 문제 → 영어 의성어 혼용 OK
- **K-Drill**: 한국어 된소리/거센소리(ㄲ/ㄸ/ㅃ/ㅋ/ㅌ) 악센트 활용
- **모던 하드코어 붐뱁**: 라이리시즘 밀도 최대, 다음절 라임 + 내재 라임
- **하드코어 트랩**: muddy/husky/distorted 딜리버리 허용

---

## Series DNA v0.1

**포지션 대비 기존 Wavvy 시리즈:**
- 12-00 `Korean Afrobeats` — 한낮 정오, 공격적 그루브
- 13-00 `FEEL GOOD R&B · URBAN NEO-SOUL` — 봄 오후 커플 설렘
- 14-00 `Soft R&B` — 오후 햇살에 멍해지는 시간
- 15-00 `FUNKY R&B · URBAN NEO-SOUL` — 오후 3시 바람 좋은 드라이브
- 16-00 `감성 락` — 교차점
- 18-00 `Neo-Soul` — 퇴근길 전환
- 21-00 `City Pop` — 밤산책 사색
- 22-00 `Ambient Slow R&B` — 수면 전 이완
- **20-00 `HARD HIP-HOP · DARK CITY · NIGHT GRIND`** — 퇴근 직후 해방·그라인드·도시 질주

**시간 서사**: 18-00(퇴근 전환) → 19-00(공백) → **20-00(해방 피크)** → 21-00(사색) → 22-00(이완)

**포지셔닝 하이브리드:**
- **After Hours 프레임** (감정): 퇴근 후 해방감 · 자유 · 밤의 시작
- **Dark City 미학** (시각): 네온 · 창밖 야경 · 밤 운전 · 도시 질주

**차별점:**
1. **Wavvy 첫 "빡센" 시리즈** — 기존 감성/릴랙스 위주에서 이탈
2. **4축 구조** — 단일 톤이 아닌 스펙트럼 (Rage ↔ Drill ↔ Boom bap ↔ Hard Trap)
3. **"시간대 × 힙합" 글로벌 블루오션** — Ryan Celsius "Trappin in Japan" 외 유일 선례 부재
4. **Korean 로컬 샘플 전략** (KC K-FLIP+ 모델) — 한국 인디 록/힙합/발라드 샘플링으로 현지화

**시그니처 요소 (4축 공통):**
- BPM 기본 트랙: **86-95 (boom bap)**, **140-155 (rage/trap)**, **140-145 (drill)**
- Key 분포: Minor 우세 (다크 무드), Major는 rage/anthem 곡
- 보컬: Male-dominant (한국 하드 힙합 남성 중심), Chest voice 주력
- 테마: 퇴근 · 도시 · 그라인드 · 야경 · 해방 · 내면 · 자기 서사

---

## Style Templates (4축)

### Style A: Rage / Aggressive Trap (4곡)

**대상**: KC 레이블(Sik-K·HAON·Vangdale) + FDT 크루(Ourealgoat·LIL GIMCHI·호미들) 계열

**BPM**: 140-160 (허브 148-152)
**Key**: Minor 우세, Anthem은 Major
**보컬**: Male, pitch-up/screamed auto-tune, 영어 의성어 애드립 허용

**Suno Style Prompt**
```
Korean rage trap, 148 BPM, distorted 808 bass with pitch slides hard clipped, bell lead synth, supersaw 7-voice detuned, pitched-up vocal chop sample top layer, rolling hi-hats with 32nd triplet roll, screamed autotune male rap vocal, pitched-up ad-libs high density, mosh pit energy aggressive, raw lo-fi saturation, locked drum pattern throughout no fills no switch-up, hard rap only no singing, no arp synth no stutter loop, rapping in Korean, sharp articulation
```

**EXCLUDE**: `k-pop, melodic singing, four-on-the-floor, drum fill, double-time switch, sung hook, arpeggiated synth, electronic riser, synth FX, EDM FX`

**가사 구조 권장**
- Intro → Hook (16 bar) → Verse 1 (16 bar) → Hook → Verse 2 → Hook → Outro (cut)
- 영어 슬로건 훅 + 한국어 버스
- 애드립 영어 위주 (`Yuh!` `What!` `Huh!` `Slatt!`)
- 가사 밀도 낮음, 반복 훅 중심

### Style B: K-Drill (NY/UK) (5곡)

**대상**: Fleeky Bang(NY drill) · Blase(UK grime+drill) · Silkybois · deadbois · NO:EL

**BPM**: 138-145 (140 공통, 하프타임 체감 70)
**Key**: Minor 주력 (D minor, F minor)
**보컬**: Male, menace/deadpan, 된소리·거센소리 악센트 활용

**Suno Style Prompt**
```
NY drill, Brooklyn drill, 140 BPM, sliding 808 bass portamento, flipped soul sample melodic hook pitched-up chop, sharp tresillo hi-hats 3+3+2 pattern, snare on beat 4 off-beat, dark orchestral strings, cinematic piano motif minor key, menacing atmosphere, aggressive male rap deadpan guttural, locked drum pattern throughout no fills no switch-up, hard rap only no singing, no arp synth no stutter loop, rapping in Korean, crisp articulation
```

**EXCLUDE**: `k-pop, melodic singing, four-on-the-floor, drum fill, double-time switch, sung hook, arpeggiated synth, electronic riser, synth FX, EDM FX`

**가사 구조 권장**
- Intro (피아노 16 bar) → Verse 1 (16 bar) → Pre-Hook → Hook (8 bar) → Verse 2 → Pre-Hook → Hook
- **Bridge 금지** (Suno가 노래로 변주)
- Ad-libs: `(Grr!)` `(Bow!)` `(Pow!)` `(Gang!)` 파렌테시스 inline
- 도시 · 지역번호(서울 02, 부산 051 등) · 동네 · 음식(만두/떡볶이/닭갈비) 한국적 위협 은유
- 한국어 90% + 영어 hook 10%

### Style C: 모던 하드코어 붐뱁 (7곡)

**대상**: B-Free × Hukky Shibaseki `Odyssey.1` (KHA 2025 Album of Year) · Owen · Huckleberry P · Paloalto · Simon D 포세 · Kid Milli · QM

**BPM**: 86-95 (88-92 sweet spot)
**Key**: 다양
**보컬**: Male, chest voice dominance, 다음절 라임 · 내재 라임

**Suno Style Prompt**
```
modern hardcore boom bap, 한국 힙합, 90 BPM, chopped 70s-80s soul or ballad sample loop with vinyl crackle, dusty drum break MPC swing 58%, warm upright bassline, acoustic kick snappy snare rim shot, DJ scratches, confident lyrical male rap in Korean chest voice multi-syllable rhymes, analog saturation head-nodding groove, locked drum pattern throughout no fills no switch-up, hard rap only no singing, no arp synth no stutter loop, rapping in Korean, sharp articulation
```

**EXCLUDE**: `trap, melodic singing, four-on-the-floor, drum fill, double-time switch, sung hook, arpeggiated synth, electronic riser, synth FX, EDM FX`

**가사 구조 권장**
- Intro (샘플 loop 4-8 bar) → Verse 1 (16 bar 밀도 최대) → Hook (4-8 bar 라이트) → Verse 2 → Hook → Verse 3 → Outro (hook repeat)
- Bridge 생략
- 자전 서사 (퇴근 · 야근 · 한강 · 막차 · 고시원 · 월세 · 부모님 · 학창시절) 등
- 한국어 95%+

### Style D: 하드코어 트랩 (젊은 씬) (4곡)

**대상**: ZENE THE ZILLA · Ash Island · Loopy · EK · KWAII (2010s legacy Changmo·Bewhy·Mommy Son 제외)

**BPM**: 140-150
**Key**: Minor 주력 (다크 톤)
**보컬**: Male, husky/muddy/distorted, ad-lib 미니멀

**Suno Style Prompt**
```
Korean dark hardcore trap, 142 BPM, heavy 808 sustain decay punchy kick, fast hi-hat rolls triplet, minimal clap on 3, sidechained sub bass slight distortion, dark synth pad minor piano stabs muted electric guitar chord, dark pitched vocal sample loop minor key, melancholic trap rap husky male rap vocal high-speed flow in Korean, dark tonal balance, locked drum pattern throughout no fills no switch-up, hard rap only no singing, no arp synth no stutter loop, rapping in Korean, sharp articulation
```

**EXCLUDE**: `melodic singing, k-pop, four-on-the-floor, drum fill, double-time switch, sung hook, arpeggiated synth, electronic riser, synth FX, EDM FX`

**가사 구조 권장**
- Intro → Verse 1 → Pre-Hook → Hook (chant) → Verse 2 → Pre-Hook → Hook → Outro
- **Bridge 금지** (Suno가 노래로 변주, breakdown도 제거)
- Ad-lib 미니멀 (`Skrrt!` `Uh!` 정도)
- 주제: 고립 · 불안 · 불면 · 자기혐오 · 야경 · 도시 고독 · 분노
- 한국어 85% + 영어 hook 15%

---

## Track Map v0.1 (4막 × 20곡 스켈레톤)

> **주의**: 개별 곡 제목은 **TBD** (가칭). 15-00 모델 따라 **Suno PASS 곡 누적 → DNA/배분 역추출** 워크플로우. 아래는 구조 가이드만.

### 1막 (트랙 1-5): 기 — 퇴근 직후 해방감
```
01 | Style A Rage      | 탈출 · 해방 · 점화        | TBD (가칭: ESCAPE)
02 | Style B K-Drill   | 도시 경계 · 첫 발걸음     | TBD (가칭: BLOCK)
03 | Style A Rage      | 질주 · 엑스터시          | TBD (가칭: HI BEAM)
04 | Style B K-Drill   | 지역 프라이드            | TBD
05 | Style C Boom bap  | 자기 서사 · 초상          | TBD (가칭: CLOCK OUT)
```

### 2막 (트랙 6-10): 승 — 밤 도시 탐색
```
06 | Style B K-Drill   | 네온 · 시선             | TBD
07 | Style A Rage      | 과시 · 폭발             | TBD
08 | Style D Hard Trap | 고독 · 도시 추적          | TBD
09 | Style C Boom bap  | 내면 · 회상             | TBD
10 | Style B K-Drill   | 위협 · 크루 chant       | TBD
```

### 3막 (트랙 11-15): 전 — 내면 깊이 진입
```
11 | Style C Boom bap  | 부모 · 고향             | TBD
12 | Style C Boom bap  | 야근 · 그라인드           | TBD
13 | Style B K-Drill   | 절망 · 묵은 분노          | TBD
14 | Style D Hard Trap | 불안 · 자기혐오           | TBD
15 | Style C Boom bap  | 성찰 · 라이리컬 정점       | TBD
```

### 4막 (트랙 16-20): 결 — 귀가 · 밤 마무리
```
16 | Style D Hard Trap | 불면 · 새벽 직전          | TBD (가칭: INSOMNIA)
17 | Style C Boom bap  | 마무리 서사             | TBD
18 | Style A Rage      | 재점화 · 다음날 약속       | TBD
19 | Style C Boom bap  | 막차 · 귀가길            | TBD
20 | Style D Hard Trap | 침묵 · After Hours 타이틀 | TBD (가칭: AFTER HOURS)
```

**축별 최종 배분 확인**: Style A(Rage) 4 / Style B(K-Drill) 5 / Style C(Boom bap) 7 / Style D(Hard Trap) 4 = **20곡** ✓

---

## YouTube Metadata v0.1

### Context Mode
After Hours — 퇴근 후 해방감의 폭발 + 밤 도시의 미학. 저녁 8시 직장을 나서는 순간부터 자정 전까지의 감정 아크. Rage 점화 → Drill 도시 경계 → Boom bap 내면 서사 → Hard Trap 밤 고독.

### 제목 (v0.1, 확정 대기)
```
Playlist | 20:00 | 🌃 AFTER HOURS | HARD HIP-HOP · DARK CITY · NIGHT GRIND | 밤 여덟시 · 퇴근 · 도시 | Wavvy
```

### 설명 (v0.1)
```
🌃 20:00, After Hours
직장 문을 나서는 순간, 도시가 바뀐다 - Wavvy

퇴근 후 첫 한 발자국에 바뀌는 공기의 온도,
신호등 앞에서 바뀌는 눈빛의 각도,
네온 아래 비치는 반사된 나의 표정,
밤 운전의 차창 너머 흐르는 불빛,
혼자인 시간이 비로소 열리는 순간까지 —

저녁 여덟시의 빡센 힙합.
레이지부터 드릴, 붐뱁, 다크 트랩까지 —
우리말 가사로, 밤의 온도를 기록합니다.

🌃 Track List
──────────────
(TBD — Suno PASS 20곡 확정 후 타임스탬프 기재)
```

### 해시태그/태그 (v0.1)
```
#밤힙합 #힙합플레이리스트 #퇴근힙합 #도시힙합 #한국힙합 #hiphopplaylist #nighthiphop
태그: 힙합, 한국힙합, 밤힙합, 드릴, 붐뱁, 트랩, 레이지, Wavvy
```

### 썸네일 (제작 시기: 20곡 완성 후)
- 톤: 블루아워 도시 실루엣 + 네온 악센트 + Wavvy 로고 좌상단
- 텍스트: `🌃 AFTER HOURS` (영문 대문자) / 부제 `밤 여덟시 하드 힙합`
- 분위기: 15-00 `🕶️ AFTERNOON DRIVE` 계승 (영문 메인 + 한국어 부제 포맷)

---

## LYRICS 작성 가이드 (본 시리즈 전용)

### 기본 원칙
1. **기존 Wavvy 가사 룰(설명·메타·직접 표출 금지) 제외 적용**
2. **한국어 가사 유지** (95%+ 권장, 훅·애드립 영어 OK)
3. **Chest voice + Articulation 유지** (Wavvy Brand DNA)
4. **200자 LYRICS QA 규칙 유지** (Style 필드와 별개)

### 축별 가사 가이드

**Style A (Rage)**
- 반복 훅 중심, 가사 밀도 낮음
- 영어 슬로건 OK (`ESCAPE` `GONE` `LIGHTS OUT`)
- 애드립 스택 허용 (파렌테시스 inline)
- 메타태그: `[Intro]` `[Hook]` `[Verse]` `[Drop]` `[Outro]`
- 주제: 해방, 점화, 폭발, 네온, 엑스터시

**Style B (K-Drill)**
- 위협·menace 톤, 한국적 비유 (음식·지역·동네)
- 크루 chant 훅 (`Gang Gang Gang`, `우리 동네`, `02` 등)
- Ad-libs: `(Grr!)` `(Bow!)` `(Pow!)` `(Gang!)`
- 메타태그: `[Intro]` `[Verse]` `[Pre-Hook]` `[Hook]` `[Outro]` (Bridge 금지)
- 주제: 도시, 지역, 위협, 경계, 크루

**Style C (모던 하드코어 붐뱁)**
- **라이리시즘 밀도 최대** (bar당 12-20 음절)
- 다음절 라임, 내재 라임, head rhyme
- 자전 서사 중심 (일상·가족·야근·막차·한강)
- 메타태그: `[Intro]` `[Verse]` `[Hook]` (Bridge 생략 경향)
- 주제: 서사, 성찰, 분노, 관찰, 가족, 도시 일상

**Style D (하드코어 트랩)**
- 다크한 내면, 공격성과 고립 혼합
- 멜랑콜릭 훅 + husky 플로우
- Ad-lib 미니멀
- 메타태그: `[Intro]` `[Verse]` `[Pre-Hook]` `[Hook]` `[Outro]` (Bridge 금지)
- 주제: 불면, 고독, 불안, 자기혐오, 밤 도시, 내면 압력

---

## Suno V5.5 생성 규칙 (본 시리즈 전용)

### Style 필드
- **V5.5 1,000자 한도** 활용 (200자 한도는 V4 기준, 안전 설계는 200자)
- 축별 Style Template 기본 사용, 필요 시 개별 곡 특화 태그 추가
- 한국어 락: `rapping in Korean` 맨 뒤 1개 (과부하 금지)
- **`singing` 단어 사용 금지** — Suno가 멜로딕 싱잉으로 드리프트. 힙합 시리즈는 `rapping` / `rap vocal` / `rap delivery` 사용
- **`four-on-the-floor` EXCLUDE 필수** — 힙합 본연의 syncopated 킥 패턴이 테크노/하우스식 4/4 킥(쿵짝쿵짝)으로 드리프트 방지 (4축 전부 적용)
- **`drum fill, double-time switch` EXCLUDE 필수** — 곡 중간 드럼이 갑자기 쪼개져 빨라지는 switch-up/fill 삽입 방지. Style Prompt `locked drum pattern throughout no fills no switch-up` positive 지시와 병행
- **`sung hook` EXCLUDE + `hard rap only no singing` positive 필수** — `rapping in Korean` + `melodic singing` EXCLUDE만으론 훅이 여전히 노래로 빠짐. 보컬 디스크립터에 `rap` 단어 밀도 확보 (예: `male vocal` 금지, `male rap vocal` 사용)
- **`arpeggiated synth` EXCLUDE + `no arp synth no stutter loop` positive** — 곡 중간 전자 아르페지오/스터터 반복 효과 방지. 악기 디스크립터에 `arpeggio` 단어 금지 (예: `electric guitar arpeggio` → `electric guitar chord`, `short staccato loop` → `sustained stab`)
- **`[Bridge]` 메타태그 금지** — Suno가 bridge 섹션에서 멜로딕 싱잉으로 드리프트. 4축 전부 Bridge 제거
- **`[Chorus]` / `[Pre-Chorus]` → `[Hook]` / `[Pre-Hook]`** — 힙합 맥락 강제. Chorus 용어는 Suno가 노래 섹션으로 해석. 약칭 구조에서도 `PH = Pre-Hook`, `H = Hook` 사용 (20-00 시리즈 전용 약칭)
- **`electronic riser, synth FX, EDM FX` EXCLUDE 필수** — 섹션 전환 sweep/riser, EDM 임팩트, synth 효과음 삽입 차단. 악기명(`supersaw`, `bell lead synth`, `synth pad`)은 남기고 **효과음성 FX만** 타겟팅

### LYRICS 필드
- **한국어 Hangul 직접 입력** (Romanized 비권장)
- 메타태그 `[Verse]` `[Hook]` 등 포함 (사용자 200자 LYRICS QA 규칙과 별개)
- 본 시리즈 LYRICS 200자 QA = **메타태그 제외한 순수 가사 텍스트 기준**

### EXCLUDE 필드
- **V5.5 기준 10개까지 확장** (V4는 3개 한도, 12-00 Afrobeats 선례 9개 안정)
- 축별 복붙 라인 (상기 Style Template 참조)
- 본 시리즈 기본 10종: `[축별 장르 드리프트], melodic singing, four-on-the-floor, drum fill, double-time switch, sung hook, arpeggiated synth, electronic riser, synth FX, EDM FX`

### 생성 전략
1. 동일 Prompt로 **2-3회 생성** → 가장 좋은 것 선택
2. Auto-tune 강도 조절: 의도적 distortion 원할 시 프롬프트에 명시
3. Fleeky Bang·KC·Ash Island 등 실존 아티스트명 **직접 명기 금지** (Suno 거부 위험)

---

## QA 체크리스트 (본 시리즈)

곡별 PASS 기준:

### 공통 (4축)
- [ ] BPM 축별 범위 준수 (A 140-160 / B 138-145 / C 86-95 / D 140-150)
- [ ] 한국어 가사 95%+ (Style A·D는 훅 영어 OK)
- [ ] 보컬 Male (Female Rage/Drill 사례 거의 없음)
- [ ] Chest voice 확인 (falsetto 제한)
- [ ] Articulation 유지 (빠른 플로우에서도 명료도)
- [ ] Style 필드 200자 내 (또는 V5.5 1,000자 내)
- [ ] LYRICS 200자 제한 (메타태그 제외 순수 가사)
- [ ] EXCLUDE 3개 이하

### 축별 특화
- **Style A (Rage)**: 왜곡 808 + bell/supersaw 확인 / 애드립 밀도
- **Style B (K-Drill)**: Sliding 808 + 3+3+2 hi-hat / menace 톤
- **Style C (Boom bap)**: 샘플 기반 + MPC swing / chest voice + 밀도
- **Style D (Hard Trap)**: 서스테인 808 + 다크 패드 / husky 딜리버리

### 시리즈 정체성 (20-00 고유)
- [ ] 저녁 8시 / 밤 / 도시 / 퇴근 / 그라인드 중 하나 이상 주제
- [ ] After Hours 감정 프레임 (해방 · 자유 · 내면)
- [ ] Dark City 시각 연상 (도시 · 네온 · 야경)

---

## 진행 상황

- [x] 4축 모델 확정 (2026-04-25)
- [x] concept.md v0.1 스캘폴딩 (본 파일, 2026-04-25)
- [ ] Suno 4축 × 1곡 1차 테스트 (프롬프트 준비 완료, `test-prompts.md` 참조)
- [ ] PASS 곡 누적 (20곡까지)
- [ ] DNA / 배분 / 4막 구조 역추출 (15-00 모델 준용)
- [ ] Track Map v1.0 확정
- [ ] WAV 리네임 (`NN__제목__영문__장르__BPM.wav`)
- [ ] 썸네일 Midjourney 3안 (20곡 완성 후)
- [ ] PACK (vfade → pack → validate)
- [ ] YouTube 메타 타임스탬프 재계산
- [ ] YouTube 업로드

---

## 참고 파일

- 딥리서치 리포트: `report/2026-04-24_hard-hiphop-4axis-musical-deep.md` (신뢰도 88%, 소스 180+, ~1,500줄)
- 팀 회의록: `../../meetings/2026-04-25_20-00-hard-hiphop-positioning.md`
- 마스터 문서:
  - `MASTER/WORKFLOWS.md` — 트랙 생성 → PACK 워크플로우
  - `MASTER/style/STYLE.md` — Style 프롬프트 구조 (본 시리즈 힙합 예외 적용)
  - `MASTER/lyrics/LYRICS.md` — LYRICS 가이드 (본 시리즈 힙합 룰 예외 적용)
  - `MASTER/cli/SPEC.md` — wavvy.py CLI
- 메모리:
  - `feedback-wavvy-genre-lyrics-rules.md` — 힙합 장르 가사 룰 예외 적용 근거
  - `feedback-wavvy-articulation.md` — Articulation 필수
  - `feedback-wavvy-lyrics-qa.md` — LYRICS 200자 제한

---

*v0.1 — 2026-04-25 스캘폴딩. Suno 테스트 결과 반영하며 v0.2 이후 진화 예정 (15-00 모델).*
